// examples/py_string.rs
// Run with: cargo run --example string
//
// This example showcases WebRust's Python-like string manipulation methods,
// with a special focus on the revolutionary splitby() method that unifies
// three common splitting patterns into one intuitive API.
//
// Core features demonstrated:
// - Python-like string methods: upper(), lower(), title(), capitalize(), swapcase()
// - Trim operations: trim(), ltrim(), rtrim()
// - Search & validation: startswith(), endswith(), find(), count()
// - Content checks: isalpha(), isdigit(), isalnum(), isupper(), islower()
// - Padding & formatting: zfill(), ljust(), rjust(), center()
// - Prefix/suffix removal: removeprefix(), removesuffix()
// - ⭐ splitby() - THE STAR: One method, three patterns (delimiter/whitespace/lines)
//
// splitby() patterns:
// * splitby(",")   - Split by delimiter (like Python's split(","))
// * splitby("")    - Split by whitespace (like Python's split())
// * splitby("\n")  - Split by newlines (like Python's split("\n"))
//
// Performance notes:
// * All methods are #[inline] for zero-cost abstractions
// * Pattern matching resolved at compile-time
// * No runtime overhead compared to native Rust
// * Fluent chaining: .splitby().join() for elegant pipelines
//
// Tips:
// * Use splitby("") for tokenizing whitespace-separated text
// * Chain operations: "a,b,c".splitby(",").join(" → ")
// * Most methods are aliases to Rust std, others are custom implementations
// * All methods preserve Rust's type safety and compile-time guarantees
//
// Real-world applications:
// * CSV processing: row.splitby(",")
// * Tag processing: tags.splitby(",").iter().map(|t| t.trim())
// * Path parsing: path.splitby("/")
// * Log parsing: log.splitby("\n")
