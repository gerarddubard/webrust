...
fn main() {
    ...
    println("@(limegreen)✨ Now with ultra-simplified split_by() method - Python syntax!");
    ...
    // ===================== NOUVELLE SECTION split_by - STAR OF THE SHOW =====================
    println("\n@(gold, bold)⭐ split_by - The Python-like Revolution:");
    println("@(limegreen, bold)🎯 ONE method, THREE patterns - Just like Python!");

    // Pattern 1: Delimiter split_by
    println("\n@(cyan, bold)📋 Pattern 1 - Delimiter split_by:");
    println("@(yellow) 🐍: \"python,rust,go\".split(\",\")                # ['python', 'rust', 'go']");
    println("@(deepskyblue) 🧩: \"python,rust,go\".split_by(\",\")              // QUASI-IDENTICAL! ✨");
    println("@(orange) 🦀: \"python,rust,go\".split(',').collect::<Vec<_>>()");
    let langs = "python,rust,go".split_by(",");
    println("@(darkturquoise, bold)📊 Result: {langs:c}");

    // Pattern 2: Whitespace split_by  
    println("\n@(cyan, bold)🔤 Pattern 2 - Whitespace split_by:");
    println("@(yellow) 🐍: \"hello  world\\ttab\".split()                 # ['hello', 'world', 'tab']");
    println("@(deepskyblue) 🧩: \"hello  world\\ttab\".split_by(\"\")             // QUASI-IDENTICAL! ✨");
    println("@(orange) 🦀: \"hello  world\\ttab\".split_whitespace().collect::<Vec<_>>()");
    let words = "hello  world\ttab".split_by("");
    println("@(darkturquoise, bold)📊 Result: {words:c}");

    // Pattern 3: Line split_by
    println("\n@(cyan, bold)📄 Pattern 3 - Line split_by:");
    println("@(yellow) 🐍: \"L1\\nL2\\nL3\".split(\"\\n\")                  # ['L1', 'L2', 'L3']");
    ...
    println("@(deepskyblue) 🧩: \"a,b,c\".split_by(\",\").join(\", \")            // PURE ELEGANCE! ✨");
    let chain1 = "a,b,c".split_by(",").join(" → ");
    println("@(darkturquoise, bold)📊 Result: '{chain1}'");

    println("@(yellow) 🐍: \"-\".join(\"hello world\".split())             # \"hello-world\"");
    println("@(deepskyblue) 🧩: \"hello world\".split_by(\"\").join(\"-\")        // PURE ELEGANCE! ✨");
    let chain2 = "hello world".split_by("").join("-");
    ...
    let file_parts = parts.last().unwrap().split_by(".");
    ...
    println("@(limegreen)🚀 All split_by() methods are #[inline] - Zero overhead!");
    ...
    if !input_txt.is_empty() {
        ...
        let by_comma = input_txt.split_by(",");
        let by_space = input_txt.split_by("");
        let by_newline = input_txt.split_by("\n");
    ...
    println("@(limegreen, bold)✅ Python syntax: split_by(\",\"), split_by(\"\"), split_by(\"\\n\")");
    println("@(limegreen, bold)✅ Rust performance: Zero-cost abstractions");
    println("@(limegreen, bold)✅ Fluent chaining: .split_by().join()");
