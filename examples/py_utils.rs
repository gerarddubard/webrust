...
//
// This example demonstrates WebRust's Python-like iteration capabilities
// with ranges, enumerate, and list/dict comprehensions using the intuitive
// when/then syntax.
//
// Core features demonstrated:
// - Python-like ranges: 0.to(10), 'a'.to('z'), 0.0.to(5.0).by(0.5)
// - enumerate(): Index-value pairs for any iterable
// - List comprehensions: .when(predicate).then(mapper)
// - Dict comprehensions: Automatic HashMap inference from tuples
// - Chained filtering: .when().when().then()
// - Type flexibility: Works with integers, floats, chars, vectors, arrays
//
// Syntax comparison (Python vs WebRust):
// * Python: [x**2 for x in range(10)]
// * WebRust: 0.to(10).then(|x| x * x)
//
// * Python: {x: x**2 for x in range(5)}
// * WebRust: 0.to(5).then(|x| (x, x * x))
//
// * Python: [x for x in range(20) if x % 2 == 0]
// * WebRust: 0.to(20).when(|&x| x % 2 == 0).then(|x| x)
//
// Key advantages:
// * Speaking syntax: "when" (filter) and "then" (map)
// * Auto-collect: No manual .collect() needed
// * Type inference: Vec<T> or HashMap<K,V> automatically detected
// * Chainable: .when().when().then() flows naturally
// * Universal: Works with webrust ranges, std ranges, Vec, arrays
// * Zero overhead: Compiles to identical iterator chains as manual code
//
// Performance notes:
// * All operations are lazy until .then() is called
// * Single-pass iteration for chained operations
// * Inline expansion for optimal machine code
// * No heap allocations beyond the final collection
//
// Tips:
// * Use .when() for filtering (replaces filter())
// * Use .then() for mapping and collecting (replaces map().collect())
// * Chain multiple .when() for complex conditions
// * Return tuples from .then() for HashMap comprehensions
// * Use enumerate() to get (index, value) pairs
// * Ranges support .by() for custom steps: 0.to(100).by(5)
