...
fn latex_ranges(t: &str) -> Vec<(usize, usize)> {
    ...
    while i + 1 < n {
        if b[i] == b'$' && b[i + 1] == b'(' {
            ...
            let mut d = 1;
            while i < n && d > 0 {
                if b[i] == b'(' { d += 1; } else if b[i] == b')' { d -= 1; }
                i += 1;
            }
            v.push((s, i));
        } else { i += 1; }
...
fn trans(t: &str) -> (String, Vec<proc_macro2::TokenStream>) {
    ...
    let mut fmt = String::with_capacity(n + 16);
    let mut args = Vec::new();
    while i < n {
        if r < rs.len() && i == rs[r].0 {
            fmt.push_str(&esc_braces(&t[last..rs[r].1]));
            i = rs[r].1;
            last = i;
            r += 1;
            continue;
            ...
            if let Expr::Path(ExprPath { path, .. }) = func.as_ref() {
                if path.segments.len() == 1 && (path.segments[0].ident == "println" || path.segments[0].ident == "print") {
                    if let Some(Expr::Lit(ExprLit { lit: Lit::Str(s), .. })) = args.first() {
                        ...
                        *e = syn::parse2(quote! { #name(format!(#lit #(, #a)*)) }).unwrap();
                        return;
                    }
                }
            }
...
pub fn gui(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let wrapped = quote! {{
        fn __w_json<T: webrust::serde::Serialize>(v: &T) -> String {
            fn format_json_custom(val: &webrust::serde_json::Value, depth: usize) -> String {
                match val {
                    webrust::serde_json::Value::Array(arr) => {
                        if arr.is_empty() { return "[]".into(); }
                        if arr.len() <= 3 && arr.iter().all(|v| matches!(v, webrust::serde_json::Value::Number(_))) {
                    ...
                    webrust::serde_json::Value::Object(obj) => {
                    ...
                    webrust::serde_json::Value::String(s) => format!(r#""{}""#, s),
                    webrust::serde_json::Value::Number(n) => n.to_string(),
                    webrust::serde_json::Value::Bool(b) => b.to_string(),
                    webrust::serde_json::Value::Null => "null".into(),
                }
            }
            let val = webrust::serde_json::to_value(v).unwrap_or(webrust::serde_json::Value::Null);
            let escaped = format_json_custom(&val, 0)
                .replace('&', "&amp;")
                .replace('<', "&lt;")
                .replace('>', "&gt;")
                .replace(' ', "&nbsp;");
