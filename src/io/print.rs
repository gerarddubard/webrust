...
//! # Styled Printing with Absolute Positioning
//!
//! Python-like printing with f-strings, colors, borders, and absolute positioning
//! that respects the global coordinate mode.
//!
//! ## F-String Syntax
//!
//! - `{variable}` - Variable interpolation
//! - `{expression}` - Expression evaluation  
//! - `{value:.2}` - Format specifiers
//! - `@(color, style)text@()` - Inline styling
//! - `$(LaTeX)` - Mathematical expressions
//!
//! ## Screen Constants
//!
//! - `TW` - Terminal width (half of primary screen width on Windows)
//! - `TH` - Terminal height (half of primary screen height on Windows)
//!
//! ## Examples
//!
//! ```rust,no_run
...
//! # #[gui] fn example() {
//! let x = 42;
//! println("Value: {x}, Double: {x * 2}, Formatted: {x:.2}");
//!
//! println("@(red, bold)Error:@() Something failed");
//!
//! println("Notice")
//!     .weight(2)
//!     .color("blue")
...
//! ```
//!
//! In CSS mode, negative `x` in `.at()` anchors from the right edge.
//! In Cartesian mode, (0,0) is at the center with +y pointing up.
...
pub static TW:LazyLock<u32>=LazyLock::new(||pshell("Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Screen]::PrimaryScreen.Bounds.Width",800)/2);
pub static TH:LazyLock<u32>=LazyLock::new(||pshell("Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Screen]::PrimaryScreen.Bounds.Height",600)/2);
...
pub struct PrintBox{
    ...
    x:Option<f64>,y:Option<f64>,rx:Option<f64>,sticky:bool,emitted:bool,
}

impl PrintBox{
    fn new(lines:Vec<String>,inline:bool)->Self{
        Self{lines,inline,b_top:true,b_right:true,b_bottom:true,b_left:true,weight_px:0,
            border_color:None,style:None,radius_px:0,cell_width:None,
            align: if inline { "center".into() } else { "left".into() },
            line_gap_px:None,bg_color:None,x:None,y:None,rx:None,sticky:false,emitted:false}
    }
    ...
    pub fn sticky(mut self)->Self{self.sticky=true;self}
    ...
    fn build_style(&self)->String{
        ...
        let display=if absolute{"block"}else{"inline-block"};
        let mut css=format!("display:{};white-space:normal;vertical-align:top;padding:2px 6px;",display);
        ...
        if let Some(y)=self.y{
            let pos_type=if self.sticky{"fixed"}else{"absolute"};
            css.push_str(&format!("position:{};z-index:1000;",pos_type));
            match (self.x,self.rx,current()){
                (Some(x),_,CoordMode::Cartesian)=>{
                    let lx=(*TW as f64)/2.0+x;let ty=(*TH as f64)/2.0-y;
                    css.push_str(&format!("left:{}px;top:{}px;",lx.round() as i32,ty.round() as i32));
                }
                (Some(x),_,_)=>{
                    css.push_str(&format!("left:{}px;top:{}px;",x.round() as i32,y.round() as i32));
                }
                (None,Some(r),CoordMode::Css)=>{
                    css.push_str(&format!("right:{}px;top:{}px;",r.round() as i32,y.round() as i32));
                }
                (None,Some(r),_)=>{
                    let lx=(*TW as f64)/2.0 - r;let ty=(*TH as f64)/2.0 - y;
                    css.push_str(&format!("left:{}px;top:{}px;",lx.round() as i32,ty.round() as i32));
                }
                _=>{}
            }
...
impl Drop for PrintBox{
    fn drop(&mut self){
        ...
        if absolute{
            ...
            let gap=self.line_gap_px.map(|g|format!(r#" viz-line-gap="{}""#,g)).unwrap_or_default();
            for seg in &self.lines{
