...
//! # Module `io::gui` — Lancement d'une session WebRust
//!
//! Fournit l’attribut `#[gui(...)]` pour transformer une fonction en
//! application WebRust (serveur embarqué + rendu navigateur).
//!
//! ## Exemples
//!
//! > Astuce doctest : on utilise `no_run` pour éviter d’ouvrir le serveur
//! > pendant les tests, et on **évite `fn main()`** pour ne pas dupliquer `main`.
//!
//! ### Thème par défaut
//! ```rust,no_run
//! use webrust::prelude::*;
...
//! #[gui]
//! fn demo_default() {
//!     println("<blue>Hello WebRust</blue>");
//! }
//!
//! // Lancer la démo si besoin :
//! // demo_default();
//! ```
//!
//! ### Thème personnalisé (police, taille, couleurs)
//! ```rust,no_run
//! use webrust::prelude::*;
//!
//! #[gui(Courier_New 12px white !black)]
//! fn demo_courier() {
//!     println("<white !black>Custom theme OK</>");
//! }
//!
//! // Lancer la démo si besoin :
//! // demo_courier();
...
lazy_static::lazy_static! {
    static ref GUI_STATE: Arc<Mutex<GuiState>> = Arc::new(Mutex::new(GuiState::default()));
}
...
fn serve_dynamic_css(request: Request) {
    ...
    "#,
        sc.font, sc.size, sc.bg, sc.color, sc.bg, sc.size, sc.size
    );
...
fn handle_api_post(mut request: Request, url: &str) {
    if url == "/api/input" {
        ...
        if let Ok(data) = serde_json::from_str::<Value>(&body) {
            if let (Some(id), Some(value)) = (data["id"].as_str(), data["value"].as_str()) {
                ...
                if let Some((tx, _)) = st.pending_inputs.remove(id) {
                    st.output_buffer.push(value.to_string());
        ...
        respond(
            request,
            json!({"valid":false,"error":"Invalid request"}).to_string(),
            "application/json",
        );
    } else {
        serve_404(request);
...
pub fn add_output_same_line(text: String) {
    ...
    if let Some(last) = st.output_buffer.last_mut() {
        if last.contains("class=\"webrust-line\"") {
            ...
            *last = format!(
                r#"<div class="webrust-line" style="display:block;margin:{g}px 0;">{}</div>"#,
                text
            );
        }
    } else {
...
pub fn add_output_new_line(text: String) {
    GUI_STATE.lock().unwrap().output_buffer.push(text);
}
