    ...
    while i < n {
        if i + 1 < n && b[i] == b'$' && b[i + 1] == b'(' {
            ...
            if depth > 0 {
                ...
                if display { out.push_str("$$"); } else { out.push('$'); }
                out.push_str(inner);
                if display { out.push_str("$$"); } else { out.push('$'); }
            }
            copy_from = i;
        ...
        if i + 1 < n && b[i] == b'@' && b[i + 1] == b'(' {
            ...
            for tok in styles_raw.split(',').map(|t| t.trim()).filter(|t| !t.is_empty()) {
                css.push(tok);
            }
            for (idx, tok) in css.iter().enumerate() {
                ...
                match tok.to_ascii_lowercase().as_str() {
                    ...
                    _ => { out.push_str("color:"); out.push_str(tok); }
                }
            }
            out.push_str("\">");
...
impl TableBuilder {
    ...
    where I: IntoIterator<Item = S>, S: Into<String> {
        self.headers = Some(headers.into_iter().map(|h| process_webrust_styles(&h.into()).into_owned()).collect());
        self
    }
    ...
    pub fn pivot(mut self) -> Self {
        ...
        for j in 0..cols {
            ...
            for i in 0..rows {
                if j < self.data[i].len() { row.push(self.data[i][j].clone()); } else { row.push(Cell::empty()); }
            }
            pivoted.push(row);
        }
        ...
        for (i, row) in self.data.iter().enumerate() {
            ...
            for cell in row.iter().filter(|c| c.rowspan > 0 && c.colspan > 0) {
                let (tag, class) = match cell.status {
                    ...
                    CellStatus::Value => ("td", if cell.is_number { "webrust-td-number" } else { "webrust-td-value" }),
    ...
    fn apply_merge(&mut self) {
        ...
        for row in &mut self.data {
            ...
            while j < row.len() {
                ...
                for k in (j + 1)..row.len() {
                    let c = &row[k];
                    if c.status != status || c.is_number != is_num || c.content.len() != content_len || c.content != row[j].content { break; }
                    row[k].colspan = 0;
                    span += 1;
                }
                row[j].colspan = span;
        ...
        for j in 0..cols {
            let mut i = 0usize;
            while i < rows {
                if i >= self.data.len() || j >= self.data[i].len() || self.data[i][j].rowspan == 0 || (self.data[i][j].content.is_empty() && self.data[i][j].status != CellStatus::Empty) { i += 1; continue; }
                ...
                for k in (i + 1)..rows {
                    ...
                    let c = &self.data[k][j];
                    if c.status != status || c.colspan != colspan || c.is_number != is_num || c.content.len() != content_len || c.content != self.data[i][j].content { break; }
    ...
    fn drop(&mut self) {
        if !self.rendered {
            ...
            if let Some((x, y)) = self.position {
                ...
                html.push_str("<div id=\""); html.push_str(&table_id); html.push_str("\" style=\"position:absolute;left:");
                html.push_str(b.format(left as i32)); html.push_str("px;top:");
                html.push_str(b.format(top as i32)); html.push_str("px;width:");
                html.push_str(b.format(width as i32)); html.push_str("px;height:");
                ...
                let mut html = String::with_capacity(table_html.len() + 16);
...
pub fn table<T: Serialize>(data: &T) -> TableBuilder {
    ...
    if let Value::Object(map) = &val {
        if map.values().any(|v| v.is_object()) { builder = builder.merge(); }
    }
    builder
}
    ...
    match value {
        Value::Array(arr) if !arr.is_empty() => {
            if arr.iter().all(|v| matches!(v, Value::Number(_) | Value::String(_) | Value::Bool(_) | Value::Null)) {
                ...
                for v in arr { row.push(Cell::val(v)); }
                vec![row]
                ...
                for v in arr {
                    if let Some(inner) = v.as_array() {
                        ...
                        for x in inner { r.push(Cell::val(x)); }
                ...
                for v in arr { out.push(vec![Cell::val(v)]); }
                out
            }
        }
        Value::Object(obj) => {
            ...
            for (k, v) in obj {
                let mut row = Vec::with_capacity(1 + if v.is_array() { v.as_array().map(|a| a.len()).unwrap_or(1) } else { 1 });
                row.push(Cell::key(k));
...
fn flatten_nested_object(map: &Map<String, Value>) -> Vec<Vec<Cell>> {
    ...
    let mut table = Vec::with_capacity(col_header_depth + data.values().map(|vm| vm.values().map(|v| v.len()).max().unwrap_or(1)).sum::<usize>());
    ...
    let prop_rows: Vec<Vec<Cell>> = data.into_iter().flat_map(|(prop, values_map)| {
        ...
        (0..max_rows).map(|i| {
            ...
            row.extend(sorted_key_paths.iter().map(|key_path| {
                Cell::val_from_str(&values_map.get(key_path).and_then(|v| v.get(i)).cloned().unwrap_or_default())
            }));
            row
        }).collect::<Vec<_>>()
    }).collect();
...
fn format_json(v: &Value) -> String {
    match v {
        ...
        Value::Number(n) => {
            if let Some(i) = n.as_i64() { itoa::Buffer::new().format(i).to_string() }
            else if let Some(u) = n.as_u64() { itoa::Buffer::new().format(u).to_string() }
            else if let Some(f) = n.as_f64() { ryu::Buffer::new().format(f).to_string() }
            else { n.to_string() }
        }
...
impl Cell {
    #[inline]
    fn key(s: &str) -> Self {
        Self { content: process_webrust_styles(s).into_owned(), status: CellStatus::Key, rowspan: 1, colspan: 1, is_number: false }
    }
    ...
    fn val(v: &Value) -> Self {
        ...
        Self { content: process_webrust_styles(&content).into_owned(), status: CellStatus::Value, rowspan: 1, colspan: 1, is_number }
    }

    #[inline]
    fn val_from_str(s: &str) -> Self {
        let is_number = s.trim().parse::<f64>().is_ok();
        Self { content: process_webrust_styles(s).into_owned(), status: CellStatus::Value, rowspan: 1, colspan: 1, is_number }
    }

    #[inline]
    fn empty() -> Self { Self { content: String::new(), status: CellStatus::Empty, rowspan: 1, colspan: 1, is_number: false } }
}