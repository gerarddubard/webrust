...
"#,
        ...
        let opts = match self.kind.as_str() {
            ...
            _ => {
                if self.multi_series.is_some() { self.generate_multi_series_options() } else { self.generate_standard_options() }
            }
        };
    ...
    fn generate_pie_options(&self) -> String {
        ...
        format!(r#"c.setOption({{tooltip:{{trigger:'item',formatter:'{{b}}: {{c}} ({{d}}%)'}},legend:{{show:false}},series:[{{type:'pie',radius:'50%',center:['50%','50%'],avoidLabelOverlap:true,label:{{fontSize:6,formatter:'{{b}}'}},labelLine:{{show:true,length:4,length2:2}},data:{}}}]}});"#, data)
    }

    fn generate_doughnut_options(&self) -> String {
        let data = self.create_labeled_items();
        let inner_radius = self.hole.unwrap_or(50);
        format!(r#"c.setOption({{tooltip:{{trigger:'item',formatter:'{{b}}: {{c}} ({{d}}%)'}},legend:{{show:false}},series:[{{type:'pie',radius:['{}%','70%'],center:['50%','50%'],avoidLabelOverlap:true,label:{{fontSize:6,formatter:'{{b}}'}},labelLine:{{show:true,length:4,length2:2}},emphasis:{{label:{{show:true,fontSize:8,fontWeight:'bold'}}}},data:{}}}]}});"#, inner_radius, data)
    }
    ...
    fn generate_area_options(&self) -> String {
        if self.multi_series.is_some() { return self.generate_multi_series_options(); }
        let x_labels = serde_json::to_string(&self.xlabels).unwrap();
        let data = serde_json::to_string(&self.data).unwrap();
        format!(r#"c.setOption({{tooltip:{{trigger:'axis'}},grid:{{left:30,right:10,top:10,bottom:20}},xAxis:{{type:'category',boundaryGap:false,data:{},axisLabel:{{fontSize:7}}}},yAxis:{{type:'value',axisLabel:{{fontSize:7}}}},series:[{{data:{},type:'line',areaStyle:{{}},smooth:true,itemStyle:{{color:'{}'}},lineStyle:{{width:2}}}}]}});"#, x_labels, data, self.color)
    }
    ...
    fn generate_radar_options(&self) -> String {
        ...
        let data = serde_json::to_string(&self.data).unwrap();
        format!(r#"c.setOption({{radar:{{indicator:{}}},series:[{{type:'radar',data:[{{value:{},name:'{}'}}],itemStyle:{{color:'{}'}},areaStyle:{{opacity:0.3}}}}]}});"#, indicators, data, self.name.as_ref().unwrap_or(&String::new()), self.color)
    }
    ...
    fn generate_heatmap_options(&self) -> String {
        ...
        format!(r#"c.setOption({{tooltip:{{position:'top'}},grid:{{height:'60%',top:'10%'}},xAxis:{{type:'category',data:{},splitArea:{{show:true}}}},yAxis:{{type:'category',data:{},splitArea:{{show:true}}}},visualMap:{{min:0,max:100,calculable:true,orient:'horizontal',left:'center',bottom:'5%'}},series:[{{type:'heatmap',data:{},label:{{show:true}}}}]}});"#, x_labels, y_labels_json, heatmap_data)
    }
    ...
    fn generate_funnel_options(&self) -> String {
        let data = self.create_labeled_items();
        format!(r#"c.setOption({{tooltip:{{trigger:'item',formatter:'{{b}}: {{c}} ({{d}}%)'}},legend:{{show:false}},series:[{{type:'funnel',left:'10%',top:10,bottom:10,width:'80%',sort:'descending',gap:2,label:{{show:true,position:'inside',fontSize:7}},labelLine:{{length:5,lineStyle:{{width:1}}}},itemStyle:{{borderColor:'#fff',borderWidth:1}},emphasis:{{label:{{fontSize:8}}}},data:{}}}]}});"#, data)
    }
    ...
    fn generate_candlestick_options(&self) -> String {
        ...
        let x_labels = serde_json::to_string(&self.xlabels).unwrap();
        format!(r#"c.setOption({{tooltip:{{trigger:'axis',axisPointer:{{type:'cross'}}}},grid:{{left:45,right:10,top:15,bottom:35}},xAxis:{{type:'category',data:{},scale:true,axisLabel:{{fontSize:7}}}},yAxis:{{scale:true,axisLabel:{{fontSize:7}}}},series:[{{type:'candlestick',data:{},itemStyle:{{color:'#ec0000',color0:'#00da3c',borderColor:'#8a0000',borderColor0:'#008f28'}}}}]}});"#, x_labels, candlestick_data)
    }
    ...
    fn generate_multi_series_options(&self) -> String {
        ...
        format!(r#"c.setOption({{tooltip:{{trigger:'axis'}},legend:{{data:{},top:5,textStyle:{{fontSize:7}}}},grid:{{left:30,right:10,top:30,bottom:20}},xAxis:{{type:'category',data:{},axisLabel:{{fontSize:7}}}},yAxis:{{type:'value',axisLabel:{{fontSize:7}}}},series:[{}]}});"#, legend_json, x_labels, series_json.join(","))
    }
}
...
impl Drop for Chart {
    fn drop(&mut self) {
        ...
        let div = if let Some((x, y)) = self.position {
            ...
            format!(r#"<div class="{}"><div id="{}" style="height:{}px"></div></div>"#, class, id, height)
        };
