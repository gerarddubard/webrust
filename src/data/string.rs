...
//! ## Key Features
//!
//! - **🐍 Python-like API**: Familiar methods like `splitby()`, `startswith()`, `capitalize()`
//! - **⚡ Zero-cost abstractions**: All methods are `#[inline]` for optimal performance
//! - **🎯 Intelligent splitting**: One `splitby()` method handles delimiters, whitespace, and lines
//! - **🔗 Fluent chaining**: Seamless method chaining like `"a,b,c".splitby(",").join(" → ")`
...
//! let langs = "python,rust,go".splitby(",");        // ["python", "rust", "go"]
...
//!
//! The crown jewel of this module is `splitby()` - a single method that intelligently
//! handles all splitting scenarios, just like Python's `split()`:
//!
//!
//! // Delimiter splitting
//! "csv,data,here".splitby(",")     // Split by comma
//!
//! // Whitespace splitting (empty string as pattern)
//! "hello  world\ttab".splitby("")  // Split on any whitespace
//!
//! // Line splitting  
//! "line1\nline2\nline3".splitby("\n") // Split by newlines
//!
...
//! let rows = csv.splitby("\n");
//! let headers = rows[0].splitby(","); // ["name", "age", "city"]
//!
...
//! let parts = path.splitby("/");              // ["src", "data", "string.rs"]
//! let file_parts = parts.last().unwrap().splitby("."); // ["string", "rs"]
//!
//!
//! ### Tag Cleaning
//!
...
//!
//! **Key methods:**
//! - `splitby(pattern)` - Intelligent string splitting
...
//! - Pattern matching in `splitby()` is resolved at compile time
